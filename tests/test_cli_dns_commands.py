...
from dns.state_manager import DNSStateError, DNSStateSnapshot, NetworkManagerConnectionState
...
class CliDNSCommandTests(unittest.TestCase):
    ...
    def test_dns_apply_captures_networkmanager_restore_snapshot_before_mutation(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            snapshot_path = Path(tmp) / "dns-state.json"
            resolv_conf = Path(tmp) / "resolv.conf"
            resolv_conf.write_text("# Generated by NetworkManager\nnameserver 192.0.2.53\n", encoding="utf-8")
            inventory = _inventory(resolv_conf)
            inventory = replace(inventory, manager=ResolverManager.NETWORK_MANAGER)
            snapshot = DNSStateSnapshot(
                inventory=inventory,
                network_manager_connections=(NetworkManagerConnectionState(
                    name="Home",
            ...
            with patch("cli.main.detect_resolver_manager", return_value=inventory), \
                 patch("cli.main.SystemDNSStateManager") as manager_cls, \
                 patch("cli.main.save_root_snapshot") as save_root, \
                 patch("cli.main.DNSHijackController") as controller_cls:
                manager_cls.return_value.save_state.return_value = snapshot
                controller_cls.return_value.apply.return_value = FakeApplyResult(snapshot)
                with redirect_stdout(StringIO()):
                    result = cli.main.main([
                        "dns", "apply", "--yes", "--skip-entrypoint-check",
                        "--snapshot-file", str(snapshot_path), "--resolv-conf-path", str(resolv_conf),
                    ])
            self.assertEqual(result, 0)
            save_root.assert_called_once_with([{
                "uuid": "11111111-1111-1111-1111-111111111111",
