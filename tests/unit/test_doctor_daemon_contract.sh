...
assert_contains "$DOCTOR" '"$(watchdogvpn_python)" -m diagnostics.time_check' "doctor must run the non-mutating time diagnostic"
assert_contains "$DOCTOR" '"$(watchdogvpn_python)" -m diagnostics.capture_mode_check' "doctor must run the side-effect-free capture-mode diagnostic"
