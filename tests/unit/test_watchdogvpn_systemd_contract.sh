...
assert_contains "$SERVICE" "AmbientCapabilities=CAP_NET_ADMIN CAP_NET_BIND_SERVICE CAP_NET_RAW" "daemon unit must grant confirmed ambient capabilities"
assert_contains "$SERVICE" "CapabilityBoundingSet=CAP_NET_ADMIN CAP_NET_BIND_SERVICE CAP_NET_RAW" "daemon unit must keep bounding set coherent with ambient capabilities"
...
assert_contains "$SERVICE" "StateDirectoryMode=0750" "daemon state directory must be private to service user and group"
