...
The routing model separates routing policy, capture modes, and route actions.
That separation intentionally leaves room for richer route actions, including a
...
Explicit proxy-chain and route-chain actions are deferred to a dedicated v2
workstream before the final Full CLI and v2.0.0 release.

Until that dedicated workstream lands, the current runtime keeps the route-action
...
dedicated v2 chain workstream defines, implements and validates the complete
contract.

## Future Chain Contract

ADR 0008 defines the accepted chain route-action contract.

Before chain actions can be accepted, the dedicated v2 chain workstream must define:
...
- route, DNS, teardown and failure behavior kept correct with an external VPN
  down or up where applicable.

## Consequences

- v2.0.0 still includes proxy-chain/route-chain work, but in its own dedicated
  workstream instead of as a partial routing-model add-on.
- The routing model avoids a half-built chain feature that could misroute traffic
  or hide DNS behavior.
- Existing route actions remain stable and inspectable.
- The dedicated chain workstream can add the new route-action namespace without
  rewriting the routing/capture contract.
